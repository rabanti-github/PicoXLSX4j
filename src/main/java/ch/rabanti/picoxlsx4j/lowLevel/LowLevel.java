...
public class LowLevel {
  ...
  private String createColsString(Worksheet worksheet) {
    if (worksheet.getColumns().size() > 0) {
      ...
      for (Map.Entry<Integer, Column> column : worksheet.getColumns().entrySet()) {
        ...
        sb.append("<col customWidth=\"1\" width=\"").append(column.getValue().getWidth()).append("\" max=\"")
            .append(col).append("\" min=\"").append(col).append("\"").append(hidden).append("/>");
      }
  ...
  private String createMruColorsString() {
    ...
    if (tempColors.size() > 0) {
      sb.append("<mruColors>");
      for (String tempColor : tempColors) {
        sb.append("<color rgb=\"").append(tempColor).append("\"/>");
      }
      sb.append("</mruColors>");
      return sb.toString();
    } else {
  ...
  private String createRowString(List<Cell> columnFields, Worksheet worksheet) {
    ...
    StringBuilder sb = new StringBuilder(
        43 * colNum); // A row string size is according to statistics (random value) 43 times the column number
    //StringBuilder sb = new StringBuilder();
    if (colNum > 0) {
      sb.append("<row r=\"");
    ...
    for (Cell columnField : columnFields) {
      ...
      if (item.getDataType() != Cell.CellType.EMPTY) {
        sb.append("<c").append(tValue).append("r=\"").append(item.getCellAddress()).append("\"").append(sValue)
            .append(">");
  ...
  private String createSheetProtectionString(Worksheet sheet) {
    ...
    while (itr.hasNext()) {
      item = itr.next();
      temp = item.getKey()
          .name();// Note! If the enum names differs from the OOXML definitions, this method will cause invalid OOXML entries
  ...
  private Document createStyleSheetDocument() throws IOException {
    ...
    if (this.workbook.getWorkbookMetadata() != null) {
      if (!Helper.isNullOrEmpty(mruColorString) && this.workbook.getWorkbookMetadata().isUseColorMRU()) {
        sb.append("<colors>");
  ...
  private String createStyleXfsString() {
    ...
    for (Style style : styles) {
      ...
      if (style.getCellXf().getHorizontalAlign() != CellXf.HorizontalAlignValue.none
          ...
          || style.getCellXf().getAlignment() != CellXf.TextBreakValue.none || textRotation != 0) {
  ...
  private Document createWorkbookDocument() throws IOException {
    if (this.workbook.getWorksheets().isEmpty()) {
      throw new RangeException(
          "UnknownRangeException",
          "The workbook can not be created because no worksheet was defined.");
    }
    StringBuilder sb = new StringBuilder();
    sb.append(
    ...
    for (int i = 0; i < this.workbook.getWorksheets().size(); i++) {
      ...
      sb.append(id);
      sb.append("\" name=\"")
          .append(LowLevel.escapeXMLAttributeChars(this.workbook.getWorksheets().get(i).getSheetName())).append("\"/>");
    }
    sb.append("</sheets>");
    sb.append("</workbook>");
  ...
  private Document createWorksheetPart(Worksheet worksheet) throws IOException {
    ...
    for (List<Cell> item : celldata) {
      line = this.createRowString(item, worksheet);
      sb.append(line);
    }
    sb.append("</sheetData>");
  ...
  public Document createXMLDocument(String rawInput, String title) throws IOException {
    try {
      ...
      throw new IOException(
          ...
          "There was an error while creating the XML document. Please see the inner exception.",
          e);
    }
  }
  ...
  private List<List<Cell>> getSortedSheetData(Worksheet sheet) {
    ...
    for (Map.Entry<String, Cell> stringCellEntry : sheet.getCells().entrySet()) {
      entry = stringCellEntry;
  ...
  public void save() throws IOException {
    try {
      ...
      throw new IOException(
          "SaveException",
          "There was an error while creating the workbook document during saving to a file. Please see the inner exception:"
              + e.getMessage(),
          e);
    }
  }
  ...
  public void saveAsStream(OutputStream stream) throws IOException {
    try {
      ...
      for (int i = 0; i < this.workbook.getWorksheets().size(); i++) {
        ...
        doc = this.createWorksheetPart(sheet);
      ...
      p.addPart("xl/sharedStrings.xml", "application/vnd.openxmlformats-officedocument.spreadsheetml.sharedStrings+xml",
          this.createSharedStringsDocument());
      p.addPart(
          "xl/workbook.xml",
      ...
      throw new IOException(
          "SaveException",
          "There was an error while creating the workbook document during writing to a stream. Please see the inner exception:"
              + e.getMessage(),
          e);
    }
  }
  ...
  public static byte[] createBytesFromDocument(Document document) throws IOException {
    try {
      ...
      throw new IOException(
          "ByteSteamException",
          "There was an error while creating the byte array. Please see the inner exception.",
          e);
    }
  }
